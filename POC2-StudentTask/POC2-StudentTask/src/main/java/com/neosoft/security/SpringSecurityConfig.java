...
//		  .and().withUser("admin").password("{noop}admin1234").roles("USER","ADMIN");
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.httpBasic().and().authorizeRequests().antMatchers(HttpMethod.POST, "/Project/{projId}/students")
				.hasAnyRole("ADMIN", "USER").antMatchers(HttpMethod.POST, "/project").hasAnyRole("ADMIN", "USER")
				.antMatchers(HttpMethod.POST, "/**").hasAnyRole("ADMIN") // for all end points with post
				.antMatchers(HttpMethod.GET, "/**").hasAnyRole("ADMIN").antMatchers(HttpMethod.PUT, "/project/**")
				.hasAnyRole("ADMIN").antMatchers(HttpMethod.DELETE, "/students/**").hasAnyRole("ADMIN").and().csrf()
				.disable();
	}
